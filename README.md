...
## 重要!!
　PASOPIA7あるあるの電源ユニット部フィルムコンデンサの破裂による修理で適切な品に交換されていない場合、電源電圧は正常(+5V)なのに信号電圧が異常(+10V程度)な状態となることがあるようです。
...
　5)～EEEBh:機械語プログラムをLOAD後は、すべてのコマンドが使えません。EDC9h以降までLOADされた場合は強制的に実行番地にジャンプを試みます。
